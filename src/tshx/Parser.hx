		...
		var r = switch stream {
			...
			case [{def: TKeyword(TsImport)}]:
				switch stream {
					...
					case [e = reference()]:
						switch stream {
							case [{ def : TIdent("from") }, { def : TString(path) }]:
								DImport([e], path);
							case [{def: TComma}, {def: TLBrace}, el = psep(TComma, reference), { def:TRBrace }, { def : TIdent("from") }, { def : TString(path) }]:
								DImport([e].concat(el), path);
						}
				}
